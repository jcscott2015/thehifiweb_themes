 ...
 *   entirety and paste it here, changing the prefix from theme_ to canadianhifi_.
 *   For example:
 *
 *     original: theme_breadcrumb()
 *     theme override: canadianhifi_breadcrumb()
 *
 *   where canadianhifi is the name of your sub-theme. For example, the
...
if (theme_get_setting('canadianhifi_fixed')) {
 ...
 */
function canadianhifi_theme(&$existing, $type, $theme, $path) {
 ...
 */
function canadianhifi_preprocess(&$vars, $hook) {
	// Initiate currency in CAD.
	if (!isset($_SESSION['currency_type'])) $_SESSION['currency_type'] = "CAD";
  
	switch($hook) {
	case 'webform_form':
		// Add custom class to "Sign up for news and promotions" webform submit button.
		// Used by canadianhifi_button() below.
		if($vars['form']['form_id']['#value'] == 'webform_client_form_1') {
			if(isset($vars['form']['submit']['#attributes']['class'])) {
				...
				$vars['form']['submit']['#attributes']['class'] = 'red-grey ' . $curr_class;
			} else {
				$vars['form']['submit']['#attributes']['class'] = 'red-grey';
			}
...
*   An array of variables to pass to the theme template.
*/
function canadianhifi_preprocess_node(&$vars, $hook) {
  $node = $vars['node'];
...
function canadianhifi_preprocess_comment(&$vars, $hook) {
 ...
 */
/* -- Delete this line if you want to use this function
function canadianhifi_preprocess_block(&$vars, $hook) {
  $vars['sample_variable'] = t('Lorem ipsum.');
}
// */
 ...
 * @ingroup themeable
 */
function canadianhifi_uc_product_image($images, $teaser = 0, $page = 0) {
  // get nid
 ...
 * @ingroup themeable
 */
function canadianhifi_button($element) {
  ...
  if (isset($element['#attributes']['class'])) {
	if ((isset($element['#attributes']['override'])) && ($element['#attributes']['override'] > 0)) {
		...
		$element['#attributes']['class'] = 'red ' . $form_class . ' ' . $element['#attributes']['class'];
...
function canadianhifi_submit($element) {
...
*/
function canadianhifi_preprocess_search_theme_form(&$vars, $hook) {
 ...
 */
function canadianhifi_preprocess_search_result(&$vars, $hook) {
  $result = $vars['result'];
  $nid = $result['node']->nid;
  $vars['nid'] = $nid;
 ...
 */
function canadianhifi_uc_catalog_product_grid($products) {
 ...
 * @ingroup themeable
 */
function canadianhifi_uc_catalog_browse($tid = 0) {
 ...
 * @ingroup themeable
 */
function canadianhifi_uc_price($value, $context, $options) {
  // Fixup class names.
 ...
 * @see uc_price()
 */
function canadianhifi_uc_product_price($price, $context, $options = array()) {
 ...
 */
function canadianhifi_views_slideshow_div_js($rows, $options, $id) {
  $hover = 'hover';
...
function canadianhifi_status_messages($display = NULL) {
  $output = '';
